...
/// <typeparam name="TSearch">查询模型的类型。</typeparam>
    ...
    where TSearch : class, new()
{
    ...
    /// 编辑对话框的标题。
    /// </summary>
     string? UpdateDialogTitle => $"{UpdateActionName}{PageTitle}";
     TSearch SearchModel { get; set; } = new();
    ...
    protected override void OnInitialized()
    {
        ...
        UpdateOperationContent = rowValue =>
                            builder => builder.Component<FormDialogLink<TUpdate>>()
                                            .Attribute(m => m.DialogTitle, UpdateDialogTitle)
                                            .Attribute(m => m.Text, UpdateActionName)
                                            .Attribute(m => m.IconName, UpdateActionIcon)
                                            ...
                                            .Attribute(m => m.ChildContent, model => async builder =>
                                            {
                                                var detail = await GetDetailData(rowValue, Key);
                                                var update = MapDetailToUpdateProvider(detail);
                                                builder.AddContent(0, UpdateFormContent?.Invoke(update!));
                                            })
                                            .Close();
    ...
    {
        TableDataSource = await ListDataSourceProvider.Invoke(SearchModel);
        StateHasChanged();
    }
